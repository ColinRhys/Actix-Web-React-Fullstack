...
use std::{collections::{HashSet, HashMap}, future::Future, pin::Pin};
...
impl Claims {
    pub fn validate_permissions(&self, required_permissions: &HashSet<String>) -> bool {
        self.permissions.as_ref().map_or(false, |permissions| {
            println!("The permissions from the Bearer Token is {:?}", permissions);
...
impl FromRequest for Claims {
    ...
    fn from_request(
        ...
        let auth_domain = std::env::var("AUTH0_DOMAIN").expect("AUTH0_DOMAIN");
        let auth_audience = std::env::var("AUTH0_AUDIENCE").expect("AUTH0_AUDIENCE");
        ...
        Box::pin(async move {
            ...
            let jwks = Client::new()
                ...
                .json::<JwkSet>()
                .await
                .expect("Error turing response from client to Auth0 into JwkSet");
            let jwk = jwks
                .find(&kid)
